...
const generatePDFPage = (doc, copyType, invoiceNumber) => {
  ...
  const headerTable = [
    ...
    ['Address:1/400 North Street M Duraisamypuram', '', `${copyType}`],
    ['Mamsapuram (po) ', '',`Invoice Number: ${invoiceNumber}`],
    ['Sivakasi (TK)', '',`Date: ${selectedDate.getDate().toString().padStart(2, '0')}-${monthNames[selectedDate.getMonth()]}-${selectedDate.getFullYear()}`  ],
    ['Virudhunagar - 626124', '', 'GSTIN : 33AAZFG9211C1ZD'],
    ['State: 33-Tamil Nadu', '', ''],
...
const CustomerCopy = async () => {
 if (cart.length === 0) {
  ...
  const headerTable = [
    ['GKM TRADERS', '', 'TAX INVOICE'],
    ['Address:1/400 North Street M Duraisamypuram', '', 'CUSTOMER COPY'],
    ['Mamsapuram (po) ', '',`Invoice Number: ${invoiceNumber}`],
    ['Sivakasi (TK)', '',`Date: ${selectedDate.getDate().toString().padStart(2, '0')}-${monthNames[selectedDate.getMonth()]}-${selectedDate.getFullYear()}`  ],
    ['Virudhunagar - 626124', '', 'GSTIN : 33AAZFG9211C1ZD'],
    ['State: 33-Tamil Nadu', '', ''],
